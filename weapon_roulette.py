...
st.markdown("<p style='text-align: center;'>ボタンを押すと、ブキがランダムに選ばれるよ！</p>", unsafe_with_html=True)

# ブキが1つも見つからない場合の警告
if not weapons:
    st.error("作った人が何かを間違えています、文句言ってやってください。")
else:
    ...
    with col2:
        ...
        if st.button("ルーレットを回す！", use_container_width=True):
            ...
            # 決定したブキの名前を表示（中央寄せ）
            placeholder_text.markdown(f"<h2 style='text-align: center;'>【 {final_weapon} 】</h2>", unsafe_with_html=True)
